...
#define inc_counter_np(counter) \
  ...
  address generate_verify_oop() {
    ...
    __ mov(c_rarg3, (uint64_t) Universe::verify_oop_mask());
    __ andr(c_rarg2, r0, c_rarg3);
    __ mov(c_rarg3, (uint64_t) Universe::verify_oop_bits());
  ...
  private:
    ...
    void pre2(RegisterOrConstant i, RegisterOrConstant len) {
      ...
      if (i.is_register()) {
        ...
        mov(Rj, (u_int64_t)i.as_constant());
        sub(Rj, Rj, len);
      }
      // Rj == i-len
    ...
    void post2(RegisterOrConstant i, RegisterOrConstant len) {
      block_comment("post2");
      if (i.is_constant()) {
        mov(Rj, (u_int64_t)i.as_constant()-len.as_constant());
      } else {
        sub(Rj, i.as_register(), len);
      }
