...
class Symbol : private SymbolBase {
 ...
 private:
  ...
  void* operator new (std::size_t size, int len, TRAPS) throw();
  void* operator new (std::size_t size, int len, Arena* arena, TRAPS) throw();
  void* operator new (std::size_t size, int len, ClassLoaderData* loader_data, TRAPS) throw();
