...
class markOopDesc: public oopDesc {
 ...
 public:
  ...
  enum { lock_mask                = right_n_bits(lock_bits),
         ...
         sgx_mark                 = right_n_bits(sgx_bits),
         sgx_mark_in_place        = sgx_mark << sgx_shift
#endif
  };
...
#define enclave_obj (OneBit << sgx_shift)
#define mark_gc (OneBit << (sgx_shift + 1))

  bool is_mark_enclave_gc() const {
    return mask_bits(value(), sgx_mark_in_place) == mark_gc;
  }

  bool is_mark_enclave_obj() const {
    return mask_bits(value(), sgx_mark_in_place) == enclave_obj;
  }

  markOop set_sgx_gc_mark() const {
    return markOop(value() | mark_gc);
  }

  markOop set_enclave_obj_mark() const {
    return markOop(value() | enclave_obj);
  }

  markOop clear_sgx_gc_mark() const {
      return markOop(value() & ~mark_gc);
  }

  markOop clear_enclave_obj_mark() const {
      return markOop(value() & ~enclave_obj);
  };
